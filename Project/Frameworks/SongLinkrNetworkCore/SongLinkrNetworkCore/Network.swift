...
public final class Network: @unchecked Sendable {
    ...
    public enum DataLoaderError: Error {
        ...
        case invalidURL
        /**
         This error signifies that an unknown problem occurred with no description
         */
    ...
    public func request(
        ...
        do {
            ...
            // Decode the response
            return try decoder.decode(SongLinkAPIResponse.self, from: data)
            // Return
        } catch {
            // If the error has already been set
            if let error = error as? DataLoaderError {
...
extension Network.DataLoaderError: LocalizedError {
    public var errorTitle: String? {
        switch self {
            ...
            case .missingAuthToken:
                return String(
                    localized: "Missing Authorisation Token",
                    bundle: .networkCore,
                    comment: "Error message title"
                )
    ...
    public var errorDescription: String? {
        switch self {
            ...
            case .unknownNetworkProblem:
                return String(
                    localized: "Sorry, an unknown network error occurred.",
                    bundle: .networkCore,
                    comment: "Error message"
                )
        }
...
private extension Bundle {
    /// The SongLinkrNetworkCore framework bundle, so localised strings resolve against its own catalog rather than the host app's
    static let networkCore = Bundle(for: Network.self)
}
